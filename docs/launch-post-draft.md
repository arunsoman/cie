...
| 11 | "0.1.0 stable" + the `cie-mcp` distribution rename | tag `v0.1.0` + GitHub release (a22b4bf); **0.1.1** (2026-08-31): distribution renamed to `cie-mcp` (PyPI `cie` is cluster311/cie10 — an unrelated ICD-10 project); import package/CLI unchanged — **posts must use `pip install "cie-mcp[mcp]"`**; **0.1.2** (2026-08-31): spawn-robust native client entries — live-verified with the real Claude Code CLI (`claude mcp list` ✔ Connected + one-shot `claude -p` tool call) — and a verified one-click (`uv tool install` from the tag → `cie init`) documented in the README top; **0.1.3** (2026-08-31): storage selection unified — one `--backend`/`CIE_BACKEND`/auto rule across CLI and `cie-mcp` (stderr-stated, `--embedded` a permanent alias), explicit-`auto`-read-as-Neo4j bug fixed | 2026-08-31 |
